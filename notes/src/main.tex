...
\section{Multiple Variable Functions}
...
\begin{gather}
    f(x) \underbrace{U}_{Domain} \in \mathbb{R}^n \to \mathbb{R}^n
\end{gather}

This basically means, they're not nescessarily defined in an entire dimension, but on a part of it.
...
Now, we can decide that 'x = 0' and we can try and check for values on which this is true for both expressions.
